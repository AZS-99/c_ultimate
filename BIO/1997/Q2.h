...
class LifeNode {
    bool alive;
    std::vector<LifeNode*> neighbours;
public:
    explicit LifeNode(bool = false);
    LifeNode(const LifeNode&);
    friend class Life;
    friend std::ostream& operator<<(std::ostream&, const LifeNode&);
};

class Life {
    std::array<std::array<LifeNode*, 11>, 11> grid;
