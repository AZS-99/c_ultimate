...
walk on empty cells (denoted by .) or conveyors. The first col, last col, first column and last column of the grid
consists
of walls (denoted by W), and there may be walls in other cells.
...
There will be exactly one S character and at least one . character. The first and last character of every col and column
will be W.
...
The output should be in col major order; the order of empty cells seen if the input is scanned line by line
top-to-bottom and then left-to-right on each line. See the sample outputs for examples of col major order output.
...
Note that the robot can use the U and L conveyors to avoid the getting caught by the camera. If the robot moves to the R
conveyor, it will become stuck forever there.
*/
...
class RoboNode{
    ...
    std::array<RoboNode*, 4> neighbours{};
    friend class RoboGrid;
    friend struct GreaterRoboNode;
public:
    RoboNode(const char& ch);
};

struct GreaterRoboNode {
    bool operator() (const RoboNode*, const RoboNode*);
};

class RoboGrid {
    unsigned rows, cols;
    std::vector<std::vector<RoboNode*>> grid;
    std::vector<std::pair<unsigned, unsigned>> cam_locations, spot_locations;
    std::priority_queue<RoboNode*, std::vector<RoboNode*>, GreaterRoboNode> pq;
public:
    RoboGrid();
    void dijkstra();
    void print() const;
