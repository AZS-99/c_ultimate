...
One possible optimal choice of c is 14, which would require your first friend to walk to position 11 (taking 4 × 1 = 4
seconds) and your second friend to walk to position 16 (taking 4 × 4 = 16 seconds), for a total of 20 seconds.
...
class WalkMan {
    ...
    std::vector<unsigned> rate_;
    std::vector<unsigned> range_;
    unsigned num_students_;
    unsigned min_;
    unsigned max_;
    unsigned min_dist_;
    unsigned GetTime(const int&);
    unsigned GetStudentTime(const int&, const unsigned&);
public:
    WalkMan();
};
